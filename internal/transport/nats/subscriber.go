...
func (s *Subscriber) Subscribe() error {
	...
	_, err = s.nc.Subscribe(SubjectVPCDefaultGet, func(msg *nats.Msg) {
		...
		if err == nil && vpc != nil {
			...
			l.Info("Resolved default VPC for response", zap.String("vpc_id", vpc.ID))
		} else if err != nil {
	...
	_, err = s.nc.Subscribe(SubjectResourceAttach, func(msg *nats.Msg) {
		...
		err := s.netService.AttachResourceToSubnet(ctx, req.TenantID, req.ResourceARN, req.TargetVPCID, req.TargetSubnetID)
		...
		resp := domain.AttachResourceResponse{
			...
			PrivateIP:     privateIP,
			Message:       "Resource successfully attached to subnet",
		}
		if err != nil {
